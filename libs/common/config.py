...
class Settings:
    ...
    camera_config_path: str = _env("CAMERA_CONFIG_PATH", "config/camera_configs.json")
    snapshot_dir: str = _env("SNAPSHOT_DIR", "vision_snapshots")
    frame_store_dir: str = _env("FRAME_STORE_DIR", "vision_frames")
    frame_queue_max_depth: int = int(_env("FRAME_QUEUE_MAX_DEPTH", "24"))
    ...
    inference_max_frame_age_seconds: float = float(_env("INFERENCE_MAX_FRAME_AGE_SECONDS", "2.5"))
    ...
    yolo_confidence: float = float(_env("YOLO_CONFIDENCE", "0.4"))
    ...
    detection_categories: str = _env("DETECTION_CATEGORIES", "person,vehicle")
    detection_min_box_area: float = float(_env("DETECTION_MIN_BOX_AREA", "0.0025"))
